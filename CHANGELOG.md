...
- Add Grafana managed identity to support Azure Monitor data sources [@sossickd](url)
- Add Grafana managed identity Azure role assignments for core workspaces [@jamurtag](url)
- Add Grafana managed identity output to support custom role assignments and data sources [@jamurtag](url)
...
- Update RBAC to allow for identities within tenant [@dutsmiller](url)
...
- Remove azure_environment variable from cert-manager and external-dns as they can now inherit the top-level variable. [@dutsmiller](url)
