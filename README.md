...
| `placement_group_key` | If specified the node group will be added to a proximity placement group created for the key in the zone, this needs `single_vmss` to be `false`. The key needs to be lowercase alphanumeric and unique to the AKS cluster . | `bool`                            | `string`    |
