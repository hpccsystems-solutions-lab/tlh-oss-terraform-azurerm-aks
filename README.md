...
| `cluster_version`                                                   | Kubernetes version to use for the Azure Kubernetes Service managed cluster; versions `1.25` (**EXPERIMENTAL**), `1.24` or `1.23` (DEPRECATED) are supported.                                                                                                                                                                                                                                                                     | `string`                                   |                   |                                                                                                                                                                                                                                                                                                            |
| `sku_tier`                                                          | Pricing tier for the Azure Kubernetes Service managed cluster; \"free\" & \"paid\" are supported. For production clusters or clusters with more than 10 nodes this should be set to `paid` (see [docs](https://learn.microsoft.com/en-us/azure/aks/free-standard-pricing-tiers)).                                                                                                                                   | `string`                                   |                   |
| `sku_tier_paid`                                                     | **DEPRECATED** - If the cluster control plane SKU tier should be paid or free; the paid tier has a financially-backed uptime SLA. This has been replaced by `sku_tier`.                                                                                                                                                                                                                                             | `bool`                                     |                   |
...
| `control_plane_logging_external_workspace_different_resource_group` | **DEPRECATED** - If true, the log analytics workspace referenced in `control_plane_logging_external_workspace_id` is created in a different resource group to the cluster.                                                                                                                                                                                                                                          | `bool`                                     | `false`           |
...
| `node_os`             | OS to use for the node group(s), `ubuntu`, `windows2019`, `windows2022` (**EXPERIMENTAL**) & `windows` (**DEPRECATED**) are supported, [Windows node support](#windows-node-support) is not guaranteed but best-effort and needs manually enabling.                                                                      | `string`                                   | `"ubuntu"`  |
